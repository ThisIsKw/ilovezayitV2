...
local function salvarPosicaoBotao(pos)
    ...
    pcall(function()
        if type(makefolder) == "function" then
            pcall(makefolder, "moon")
            pcall(makefolder, "moon/mm2")
        end
        writefile(BOTAO_POS_FILE, data)
        saved = true
    end)
    if not saved then
        pcall(writefile, "botao_pos.txt", data)
    end
end
...
local function carregarPosicaoBotao()
    ...
    local data = nil
    pcall(function()
        data = readfile(BOTAO_POS_FILE)
    end)
    if not data or data == "" then
        pcall(function()
            data = readfile("botao_pos.txt")
        end)
    end
...
local function PegarArmaNoChao()
    ...
    for _, v in pairs(workspace:GetDescendants()) do
        if v.Name == "GunDrop" and v:IsA("BasePart") then
            -- Tenta disparar o TouchInterest da arma diretamente
            -- Isso simula o toque sem mover o personagem
            local touchInterest = v:FindFirstChildOfClass("TouchTransmitter")
            ...
            -- Fallback: mover a ARMA até o player (não o player até a arma)
            -- Desancora momentaneamente, teleporta para o HRP, re-ancora
            -- O servidor detecta o toque e faz o pickup normalmente
            local wasAnchored = v.Anchored
            v.Anchored = false
            v.CFrame   = root.CFrame
            task.wait(0.05)
            if v and v.Parent then
                v.Anchored = wasAnchored
            end
            return
        end
    end
...
local function startGetGunAuto()
    if getGunConn then return end
    -- Verifica se a arma já está no chão no momento em que o toggle é ligado
    for _, v in pairs(workspace:GetDescendants()) do
        if v.Name == "GunDrop" and v:IsA("BasePart") then
            task.wait(0.1)
            PegarArmaNoChao()
...
local function Atirar()
    ...
    local function executarTiro()
        ...
        local velMag      = alvo.Velocity.Magnitude
        local horizDist   = Vector2.new(alvo.Position.X - head.Position.X, alvo.Position.Z - head.Position.Z).Magnitude
        local tEstimado   = horizDist / BULLET_SPEED
        local sanityLimit = math.max(40, velMag * tEstimado * 1.5)
...
do
    ...
    sheriffTab:AddSlider("LeadTime", {
        Title = "Lead Time",
        Description = "Fixed offset beyond ping (0 = ping only)",
        Default = 0.01,
        Min = 0.00,
        Max = 0.15,
        Rounding = 2,
    ...
    sheriffTab:AddSlider("BulletSpeed", {
        Title = "Bullet Speed",
        Description = "Projectile speed (studs/s). Miss behind = increase. Miss ahead = decrease.",
        Default = 500,
        Min = 300,
        Max = 800,
        Rounding = 0,
    ...
    sheriffTab:AddSection("Get Gun")

    sheriffTab:AddButton({
        Title       = "Pick Up Gun",
        Description = "Instantly grab the gun on the ground",
        Callback    = function()
            PegarArmaNoChao()
        end
    })

    sheriffTab:AddToggle("GetGunAutoToggle", {
        Title       = "Auto Get Gun",
        Description = "Automatically grab the gun whenever it drops",
        Default     = false,
        Callback    = function(value)
            getGunAuto = value
            if value then
    ...
    sheriffTab:AddKeybind("GetGunKeybind", {
        Title       = "Get Gun Hotkey",
        Description = "Hotkey to grab the gun (PC only)",
        Default     = "G",
        Callback    = function()
            PegarArmaNoChao()
        end
